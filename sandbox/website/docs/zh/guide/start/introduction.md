...
大多数沙盒都是**单一用途**（浏览器、代码或 Shell），这使得文件共享和功能协调变得极其困难。例如，浏览器沙盒下载的文件往往需要先复制到外部存储，其他工具才能使用；一个 Agent 任务也可能需要多个环境准备就绪后才能运行。
...
- **Bash**：`/v1/bash/*` 处的 pipe 模式命令执行
- **代码运行时**：通过 `/v1/code/*`、`/v1/jupyter/*` 和 `/v1/nodejs/*` 执行 Python 与 JavaScript
- **MCP Hub**：`/mcp` 处的聚合服务
...
有关特定组件的详细信息，请探索侧边栏导航中的指南。如果你正在选择 Agent 应该运行在沙盒外还是沙盒内，请阅读 [Agent Call Sandbox vs In Sandbox](/zh/guide/start/agent-sandbox)。
